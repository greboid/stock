<?php
    ...
    class ItemRoutes {
        ...
        public function addRoutes(RunTimeStorage $storage): void {
            ...
            $app->get('/add/item', function() use ($smarty, $stock, $app) {
                if (count($stock->getSites()) == 0) {
                    return $app->redirect('/add/location');
                }
                try {
                    return $smarty->fetch('additem.tpl');
                } catch (Exception $e) {
            ...
            $app->post('/add/item', function() use ($smarty, $stock, $app) {
            ...
            $app->post('/edit/item/{itemid}', function($itemid) use ($smarty, $stock) {
            ...
            $app->post('/edit/item', function() use ($smarty, $stock, $app) {
                try {
            ...
            $app->get('/manage/items', function() use ($smarty, $stock) {
                try {
                    $smarty->assign('stock', $stock->getSiteStock(0));
                    return $smarty->fetch('manageitems.tpl');
                } catch (Exception $e) {
                    http_response_code(500);
                    $smarty->assign('error', $e->getMessage());
                    return $smarty->fetch('500.tpl');
            ...
            $app->post('/delete/item/{itemid}', function($itemid) use ($smarty, $stock, $app) {
